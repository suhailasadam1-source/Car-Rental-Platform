...
A complete, premium-feeling car rental web app: browse and filter a fleet, view detailed car pages, and complete a full booking flow — fully responsive from mobile to desktop.
...
The frontend works even without the backend running — src/lib/api.ts automatically falls back to local mock data (src/data/cars.json) if the API request fails, so you can preview the whole app standalone.
